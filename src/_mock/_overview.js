...
export const _appRelated = ['Tea', 'Omelette', 'Pasta', 'Rice', 'Chicken'].map(
  (name, index) => {
    const system = [2, 4].includes(index) ? 'Meal' : 'Dessert';

    const price = [2, 4].includes(index) ? _mock.number.price(index) : 0;

    const shortcut =
      (name === 'Tea' && '/assets/icons/app/tea.png') ||
      (name === 'Omelette' && '/assets/icons/app/omelette.png') ||
      (name === 'Pasta' && '/assets/icons/app/pasta.png') ||
      (name === 'Rice' && '/assets/icons/app/rice.png') ||
      '/assets/icons/app/chicken.png';
