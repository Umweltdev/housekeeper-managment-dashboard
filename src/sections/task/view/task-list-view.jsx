...
export default function UserListView() {
  ...
  const calculateTaskStatusCounts = useCallback(() => {
    const statusCounts = {
      dirty: 0,
      cleaned: 0,
      inspected: 0,
    ...
    // Check if tasks.data exists and is an array before iterating
    if (Array.isArray(tasks?.data)) {
      tasks.data.forEach((task) => {
        const statusType = task?.status?.statusType;
        if (statusType && Object.prototype.hasOwnProperty.call(statusCounts, statusType)) {
    ...
    statusCounts.total = statusCounts.dirty + statusCounts.cleaned + statusCounts.inspected;

    return statusCounts;
  }, [tasks]);

  const statusCounts = calculateTaskStatusCounts();
    ...
    <>
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{}}>
          <CustomBreadcrumbs
            heading="Room Cleaning Assignments"
            links={[{ name: 'Dashboard', href: paths.dashboard.root }, { name: 'Room Cleaning' }]}
            sx={{
              mb: { xs: 3, md: 5 },
            }}
          />
        ...
        >
          {[
            ...
            },
            {
              ...
              icon: 'solar:check-circle-bold-duotone',
              iconColor: 'success.main',
              total: statusCounts.cleaned,
              chart: {
                colors: [theme.palette.success.light, theme.palette.success.main],
                series: [statusCounts.cleaned],
              },
            },
            {
              title: 'Inspected',
              ...
              iconColor: 'info.main',
              total: statusCounts.inspected,
              chart: {
                colors: [theme.palette.info.light, theme.palette.info.main],
                series: [statusCounts.inspected],
              },
            },
            {
              title: 'Dirty',
            ...
            <Box
              key={index}
              sx={{
                flexBasis: { xs: '100%', sm: '48%', md: 'calc(25% - 16px)', lg: '100px' },
                minWidth: '100px',
                maxWidth: '100%',
                flexGrow: 1,
              }}
